...
astree* astree::adopt_children (astree* target) {
   for(unsigned int i = 0; i < target->children.size(); i++) {
      children.push_back(target->children[i]);
   }
...
void astree::tokDump (FILE* outfile, astree* tree) {
   if (tree == nullptr) {
      fprintf (outfile, "nullptr");
   } 
      else {
         tree->tokDumpNode(outfile);
   }
}

void astree::tokDumpNode (FILE * outfile) {
   fprintf(outfile, "  %-2zd %2zd.%03zd  %-4zd %-13s  %-12s\n",
      lloc.filenr, lloc.linenr, lloc.offset,
